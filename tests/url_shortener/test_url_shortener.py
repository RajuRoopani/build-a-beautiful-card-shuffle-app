...
# ---------------------------------------------------------------------------
# Storage unit tests — each test manages its own state
# ---------------------------------------------------------------------------

class TestStorage:
    def setup_method(self) -> None:
        """Clear storage before each method in this class."""
        storage.clear_store()

    def teardown_method(self) -> None:
        """Clear storage after each method in this class."""
        storage.clear_store()
    ...
    def test_save_duplicate_raises_value_error(self) -> None:
        ...
        with pytest.raises(ValueError, match="already exists"):
            storage.save_url("abc1234", "https://other.com")

    def test_increment_clicks_increases_count(self) -> None:
        storage.save_url("abc1234", "https://example.com")
        storage.increment_clicks("abc1234")
        storage.increment_clicks("abc1234")
        record = storage.get_url("abc1234")
        assert record is not None
    ...
    def test_clear_store_removes_all_entries(self) -> None:
        storage.save_url("abc1234", "https://example.com")
        storage.save_url("xyz7891", "https://other.com")
        storage.clear_store()
        assert storage.get_url("abc1234") is None
        assert storage.get_url("xyz7891") is None

    def test_all_codes_returns_saved_codes(self) -> None:
        storage.save_url("code001", "https://a.com")
        storage.save_url("code002", "https://b.com")
        codes = storage.all_codes()
        assert "code001" in codes
        assert "code002" in codes
